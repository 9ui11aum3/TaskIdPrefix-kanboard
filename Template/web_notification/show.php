<?php
// Remplace #42 → n°42 (ou préfixe configuré) dans les titres pré-construits.
// On capture le préfixe dans la closure pour éviter tout problème de portée.
$prefix = $this->prefix->getPrefix();
$applyPrefix = function ($title) use ($prefix) {
    return preg_replace('/#(\d+)\b/', $prefix . '$1', $title);
};
?>
    ...
    <div class="table-list-row table-border-left">
        <span class="table-list-title">
            ...
            <?php if ($this->text->contains($notification['event_name'], 'task.overdue') && count($notification['event_data']['tasks']) > 1): ?>
                <?= $this->text->e($applyPrefix($notification['title'])) ?>
            <?php else: ?>
                <?= $this->url->link(
                    $this->text->e($applyPrefix($notification['title'])),
